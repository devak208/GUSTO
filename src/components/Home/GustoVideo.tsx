...
export default function GustoVideo() {
  ...
  useEffect(() => {
    ...
    setVideoUrl(`https://www.youtube.com/embed/${videoId}?autoplay=1&rel=0&showinfo=0&origin=${origin}`)
  }, [])
  
  // Video thumbnail URL - use a local image for better performance
  const thumbnailUrl = "/images/video-thumbnail.jpg" // Create this thumbnail image
  ...
  const renderFallbackContent = () => (
    <div className="absolute inset-0 flex flex-col items-center justify-center bg-gray-100 dark:bg-gray-800 rounded-xl p-6">
      ...
      <h3 className="text-lg font-medium text-gray-900 dark:text-white mb-2">Video Unavailable</h3>
  ...
  return (
    <div className="relative w-full max-w-4xl mx-auto px-4">
      ...
      <div className="relative aspect-video w-full overflow-hidden rounded-xl shadow-xl">
        ...
        {!isLoaded && !hasError && (
          <div className="absolute inset-0 flex items-center justify-center bg-black/10 dark:bg-black/40 cursor-pointer" onClick={handleLoadVideo}>
            <Image 
              src={thumbnailUrl}
              alt="Video thumbnail"
              fill
